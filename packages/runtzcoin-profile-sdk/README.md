# Runtzswap Profile SDK

This package provides some handy functions to retrieve data for Runtzswap Profile system.
...
Install `@runtzswap/profile-sdk` into your project with npm:

```bash
npm install @runtzswap/profile-sdk --save
```

or yarn:

```bash
yarn add @runtzswap/profile-sdk
```
...
```js
import RuntzProfileSdk from "@runtzswap/profile-sdk";

const runtzSdk = new RuntzProfileSdk();
```
...
const runtzSdk = new RuntzProfileSdk(customProvider, 97);
```
...
const runtzSdk = new RuntzProfileSdk();
const username = runtzSdk.getUsername("0x123456789");
...
It also sets `profile_${address}` cookie containing username and avatar (now only for runtzswap.finance domain, maybe configurable in future versions)

```js
import RuntzProfileSdk from "@runtzswap/profile-sdk";

const runtzSdk = new RuntzProfileSdk();
const profile = runtzSdk.getProfile("0x123456789");
console.log(profile);
// {
//   hasRegistered: true
//   profile: {
...
const achievements = runtzSdk.getAchievements("0x123456789");
...
```JSX
import { Team, teamImages } from "@runtzswap/profile-sdk";
...
Current version of this SDK is 90% copy of existing from [runtz-frontend](https://github.com/runtzswap/runtz-frontend) repo. There are several improvements to be made in the future versions of this SDK:
